...
class TestTransactionEndpoints(TestCase):
    ...
    def test_view_wallet_transactions_success(self, mock_view_wallet_trancations):
        ...
        self.assertEqual(expected_transactions[0]["status"], transactions_list[0]["status"])
        self.assertEqual(expected_transactions[0]["transacted_at"], transactions_list[0]["transacted_at"])
        self.assertEqual(expected_transactions[0]["type"], transactions_list[0]["type"])
        self.assertEqual(expected_transactions[0]["amount"], transactions_list[0]["amount"])
        self.assertEqual(expected_transactions[0]["reference_id"], transactions_list[0]["reference_id"])
    ...
    def test_view_wallet_transactions_fail(self, mock_view_wallet_trancations):
        ...
        # ACT
        with self.app.test_client() as client:
            response = client.get("/api/v1/wallet/transactions", json={}, headers=headers)
            res = response.get_json()
    ...
    def test_deposit_to_wallet_success(self, mock_deposit_to_wallet):
        ...
        # ACT
        with self.app.test_client() as client:
            response = client.post("/api/v1/wallet/deposits", json=payload, headers=headers)
            res = response.get_json()

        # ASSERT
        self.assertEqual(response.status_code, 200)
        self.assertEqual(expected_response["status"], res.get("status"))
        self.assertEqual(expected_response["data"]["deposit"]["id"], res["data"]["deposit"]["id"])
        self.assertEqual(expected_response["data"]["deposit"]["deposited_by"], res["data"]["deposit"]["deposited_by"])
        self.assertEqual(expected_response["data"]["deposit"]["status"], res["data"]["deposit"]["status"])
        self.assertEqual(expected_response["data"]["deposit"]["deposited_at"], res["data"]["deposit"]["deposited_at"])
        self.assertEqual(expected_response["data"]["deposit"]["amount"], res["data"]["deposit"]["amount"])
        self.assertEqual(expected_response["data"]["deposit"]["reference_id"], res["data"]["deposit"]["reference_id"])
    ...
    def test_withdraw_from_wallet(self, mock_withdraw_from_wallet):
        ...
        # ACT
        with self.app.test_client() as client:
            response = client.post("/api/v1/wallet/withdrawals", json=paylod, headers=headers)
            res = response.get_json()
        ...
        self.assertEqual(expected_withdrawal["withdrawn_by"], res_withdrawal["withdrawn_by"])
