...
from app.main.service.transaction_service import view_wallet_transactions, deposit_to_wallet, withdraw_from_wallet
...
class TestTransactionService(TestCase):
    ...
    def test_deposit_to_wallet(self, mock_deposit_to_wallet):
        ...
        # Act
        response, status_code = deposit_to_wallet(customer_id, wallet_id, amount, reference_id)
        ...
        self.assertEqual(response["status"], "success")
        self.assertEqual(result, expected_response)
        mock_deposit_to_wallet.assert_called_once_with(customer_id, wallet_id, amount, reference_id)
    ...
    def test_withdraw_from_wallet(self, mock_withdraw_from_wallet):
        ...
        # Act
        response, status_code = withdraw_from_wallet(customer_id, wallet_id, amount, reference_id)
        ...
        self.assertEqual(response["status"], "success")
        self.assertEqual(result, expected_response)
        mock_withdraw_from_wallet.assert_called_once_with(customer_id, wallet_id, amount, reference_id)